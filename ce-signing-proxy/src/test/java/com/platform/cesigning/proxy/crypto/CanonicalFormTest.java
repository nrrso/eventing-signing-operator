...
class CanonicalFormTest {
    ...
    void standardAttributesSortedLexicographically() {
        CloudEvent event =
                CloudEventBuilder.v1()
                        ...
                        .withData(
                                "application/json",
                                "{\"amount\":42}".getBytes(StandardCharsets.UTF_8))
                        .build();

        byte[] canonical =
                CanonicalForm.build(event, List.of("type", "source", "subject", "datacontenttype"));
    ...
    void absentAttributesSkipped() {
        ...
        // subject and datacontenttype are absent (datacontenttype not set explicitly without data
        // content type)
        byte[] canonical =
                CanonicalForm.build(event, List.of("type", "source", "subject", "datacontenttype"));
        String result = new String(canonical, StandardCharsets.UTF_8);
    ...
    void jsonJcsCanonicalizes() {
        ...
        assertArrayEquals(
                canonical1,
                canonical2,
                "Different JSON key orderings must produce identical canonical bytes");
    }
    ...
    void jsonWithWhitespaceNormalized() {
        CloudEvent event =
                CloudEventBuilder.v1()
                        ...
                        .withData(
                                "application/json",
                                "{ \"z\" : 1 , \"a\" : 2 }".getBytes(StandardCharsets.UTF_8))
                        .build();
    ...
    void binaryDataPassedThrough() {
        ...
        String prefix =
                new String(
                        canonical, 0, canonical.length - binaryData.length, StandardCharsets.UTF_8);
        assertTrue(prefix.endsWith("data="));

        // Last 4 bytes should be the raw binary data
        byte[] dataPart = new byte[binaryData.length];
        System.arraycopy(
                canonical, canonical.length - binaryData.length, dataPart, 0, binaryData.length);
    ...
    void timestampAttribute() {
        ...
        assertTrue(
                result.contains("time=2024-01-15T10:30Z\n")
    ...
    void presentAttributesReturnsSortedCommaList() {
        ...
        String present =
                CanonicalForm.presentAttributes(
                        event, List.of("type", "source", "subject", "datacontenttype"));
