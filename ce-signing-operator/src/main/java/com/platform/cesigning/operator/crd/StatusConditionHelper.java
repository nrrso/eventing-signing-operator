...
public final class StatusConditionHelper {
    ...
    /**
     * Sets a condition in the list, returning true if the condition actually changed
     * (status transition, reason/message change, or first addition).
     */
    public static boolean setCondition(List<Condition> conditions,
                                    String type, String status, String reason, String message) {
        String now = OffsetDateTime.now(ZoneOffset.UTC).toString();
        for (int i = 0; i < conditions.size(); i++) {
            Condition existing = conditions.get(i);
            if (type.equals(existing.getType())) {
                if (!status.equals(existing.getStatus())) {
                    conditions.set(i, new ConditionBuilder()
                            .withType(type)
                            .withStatus(status)
                            .withReason(reason)
                            .withMessage(message)
