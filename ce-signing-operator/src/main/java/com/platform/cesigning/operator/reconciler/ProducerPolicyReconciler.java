...
@Workflow(
        dependents = {
            ...
            @Dependent(
                    name = "deployment",
                    type = SigningDeploymentDependentResource.class,
                    dependsOn = "secret",
                    readyPostcondition = DeploymentReadyCondition.class),
            @Dependent(name = "service", type = SigningServiceDependentResource.class),
            @Dependent(
                    name = "hpa",
                    type = SigningHpaDependentResource.class,
                    dependsOn = "deployment",
                    reconcilePrecondition = HpaEnabledCondition.class),
            @Dependent(
                    name = "pdb",
                    type = SigningPdbDependentResource.class,
                    dependsOn = "deployment"),
            @Dependent(
                    name = "service-monitor",
                    type = SigningServiceMonitorDependentResource.class)
        })
...
public class ProducerPolicyReconciler
    ...
    public UpdateControl<CloudEventSigningProducerPolicy> reconcile(
        ...
        if (secretOpt.isPresent()) {
            ...
            String keyId =
                    secretLabels != null
                            ? secretLabels.getOrDefault(
                                    SecretDependentResource.KEY_ID_LABEL, namespace + "-v1")
                            : namespace + "-v1";
            ...
            String createdAtLabel =
                    secretLabels != null
                            ? secretLabels.get(SecretDependentResource.CREATED_AT_LABEL)
                            : null;
            ...
            String previousKeyId =
                    secretLabels != null
                            ? secretLabels.get(SecretDependentResource.PREVIOUS_KEY_ID_LABEL)
                            : null;
            ...
            statusChanged |=
                    status.setCondition(
                            "RegistryPublished", "True", "Published", "Key published to registry");
            ...
            statusChanged |=
                    status.setCondition(
                            "KeyPairReady", "False", "Pending", "Waiting for Secret creation");
        }
    ...
    /**
     * Publishes the active key to the registry, handles rotation (mark old as rotating), and cleans
    ...
    private boolean publishToRegistry(
        ...
        return updateRegistryWithRetry(
                registry -> {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if ("rotating".equals(entry.getStatus()) && entry.getExpiresAt() != null) {
                            OffsetDateTime gracePeriodEnd =
                                    OffsetDateTime.parse(entry.getExpiresAt())
                    ...
                    for (PublicKeyEntry entry : entries) {
                        if (namespace.equals(entry.getNamespace())
                            ...
                            changed = true;
                            LOG.infof(
                                    "Marked stale active key %s as rotating (current: %s)",
    ...
    private void removePreviousKeyIdLabel(String namespace) {
        try {
            ...
            if (secret != null && secret.getMetadata().getLabels() != null) {
                secret.getMetadata()
                        .getLabels()
                        .remove(SecretDependentResource.PREVIOUS_KEY_ID_LABEL);
    ...
    /**
     * Updates the PublicKeyRegistry with optimistic concurrency. Reads the registry, applies the
     * modifier, and writes back. Retries on 409 Conflict up to MAX_REGISTRY_RETRIES times.
     *
    ...
    boolean updateRegistryWithRetry(RegistryModifier modifier) {
        for (int attempt = 0; attempt < MAX_REGISTRY_RETRIES; attempt++) {
            ...
            try {
                if (isNew) {
                    client.resources(PublicKeyRegistry.class).resource(registry).create();
                } else {
                    client.resources(PublicKeyRegistry.class).resource(registry).update();
                }
                return true;
            } catch (KubernetesClientException e) {
                if (e.getCode() == HttpURLConnection.HTTP_CONFLICT
                        && attempt < MAX_REGISTRY_RETRIES - 1) {
                    LOG.infof(
                            "Registry update conflict (attempt %d/%d), retrying",
